...
| **Fee-Sensitive Fallback** | Win probability is using the v3 fee-sensitive ML model (instead of the primary v2 model) because v2 saturated for this combination of inputs. Both are LightGBM classifiers; v3 is tuned for fee elasticity at the cost of overall ranking accuracy |
