...
==========================================
Optimize XGBoost hyperparameters to improve regression performance
...
def objective(trial):
    ...
    # Evaluate
    y_pred_train = model.predict(xgb.DMatrix(X_train, feature_names=SELECTED_FEATURES))
    y_pred_test = model.predict(xgb.DMatrix(X_test, feature_names=SELECTED_FEATURES))
...
print(f"\nModel trained with 1000 rounds\n")

# Predictions
y_pred_train = final_model.predict(xgb.DMatrix(X_train, feature_names=SELECTED_FEATURES))
...
model_path = MODELS_DIR / "xgboost_bidfee_optuna.json"
final_model.save_model(str(model_path))

metadata = {
    "model_type": "XGBoost Regression (Optuna Optimized)",
...
metadata_path = REPORTS_DIR / "xgboost_optuna_results.json"
...
print("\n" + "=" * 80)
print("OPTUNA XGBOOST TUNING COMPLETE")
print("=" * 80)
